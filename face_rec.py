...
def bd_init():
    '''Инициализировать BD при запуске бота'''
...
def private_bd_add(person_id, image, name):
    '''Функция добавления человека в личную библиотеку пользователя'''
...
def del_names_from_list(person_id, name):
    '''Функция поддержки private_bd_del'''
...
def private_bd_del(person_id, name):
    '''Функция для удаления человека из частной библиотеки пользователя '''
...
def public_bd_add(image, name):
    '''Функция добавления человека в публичную библиотеку'''
...
def x(input_number):
    '''поддержка экземпляра лямбды'''
    return input_number * 10


def draw_name(frame, face_location, name):
    '''Aдобавляет тег имени к изображению'''
    top, right, bottom, left = map(x, face_location)
    print(top, right, bottom, left)
    cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 0), 15)
    cv2.rectangle(frame, (left, bottom + int((right - left) / 10)), (right, bottom), (0, 0, 0), cv2.FILLED)
    font = cv2.FONT_HERSHEY_DUPLEX
    cv2.putText(frame, name, (left + 4, bottom + int((right - left) / 10) - 3), font, max(3, int((right - left) / 210)),
                (255, 255, 255), 5)


def find_faces(inputImage, person_id):
    '''Находит лица на картинке и выполняет поиск известных лиц в библиотеке'''
